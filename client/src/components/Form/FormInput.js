...
const FormInput = ({ label, input, meta: { touched, submitFailed, error }}) => {
    ...
    return (
        <div className={`field mb-6 ${hasBeenTouchedAndHasError && "error"}`}>
            ...
            <input 
                type={input.name}
                className="bg-gray-100 shadow-lg text-green-600 appearance-none text-center text-2xl border rounded w-full py-4 px-3 leading-tight focus:outline-none focus:shadow-outline" {...input} 
                autoComplete="off" 
            />
